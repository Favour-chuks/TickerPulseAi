...
export const Card: React.FC<CardProps> = ({ children, className = "", onClick }) => (
  <div 
    onClick={onClick}
    className={`bg-white dark:bg-[#18181b] border border-slate-200 dark:border-[#2d2d31] rounded-2xl p-6 shadow-sm dark:shadow-none ${onClick ? 'cursor-pointer hover:bg-slate-50 dark:hover:bg-[#27272a] transition-colors duration-200' : ''} ${className}`}
  >
    {children}
  </div>
);
...
export const Badge: React.FC<BadgeProps> = ({ children, variant = "default", className = "" }) => {
  ...
  return <span className={`px-2.5 py-1 rounded-md text-[11px] font-medium tracking-wide ${styles[variant]} ${className}`}>{children}</span>;
};
