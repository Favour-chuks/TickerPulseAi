...
const NLPView: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-[#18181b] rounded-2xl overflow-hidden animate-in fade-in duration-500 shadow-sm border border-slate-200 dark:border-none">
      {/* Header */}
      <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-[#18181b]">
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            `}>
              ...
              `}>
                {msg.role === 'user' ? <User className="w-4 h-4 text-slate-600 dark:text-zinc-300" /> : <Bot className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
              </div>
              <div className={`
      ...
      {/* Input */}
      <div className="p-4 bg-white dark:bg-[#18181b] border-t border-slate-200 dark:border-white/5">
        <div className="bg-slate-100 dark:bg-[#27272a] rounded-xl p-2 flex items-center gap-2 transition-colors">
          <input 
            ...
            className="flex-1 bg-transparent border-none focus:ring-0 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-zinc-500 px-3"
          />
          <button 
            onClick={handleSend}
            disabled={!input.trim() || isLoading}
            className="bg-white dark:bg-zinc-100 hover:bg-slate-50 dark:hover:bg-white text-slate-900 dark:text-black p-2 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
